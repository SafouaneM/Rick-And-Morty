            ...
            <React.Fragment>
                ...
                {this.state.character ? (
                    <div className="container">
                        ...
                        {this.state.character.slice(0,6).filter(character => character.name.includes(this.state.search)).map(character =>( //this.state.character ? () : () == if state is existing pass left and if it doesnt right
                            <Card
                                name={character.name}
                                id={character.id}
                                key={character.id}
